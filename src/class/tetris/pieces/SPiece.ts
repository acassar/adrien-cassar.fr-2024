...
export class SPiece extends Piece {
	...
	getFuturePositions(): [PieceBlock, number][] {
		const initialPositions: [PieceBlock, number][] = [
			[this.pieceBlocks[0], this.pieceBlocks[0].position - this.gridSize.x * 2],
			[this.pieceBlocks[1], this.pieceBlocks[1].position - this.gridSize.x - 1],
	...
	constructor(gridSize: GridSize) {
		...
		const pieceOrigin = Math.floor(gridSize.x / 2 - 1) + gridSize.x;
		super([new PieceBlock(pieceOrigin, color), new PieceBlock(pieceOrigin + 1, color), new PieceBlock(pieceOrigin + 1 - gridSize.x, color), new PieceBlock(pieceOrigin + 2 - gridSize.x, color)], gridSize);
	}
}